...
<body class="bg-slate-50 text-slate-800 antialiased font-sans">
        ...
        <div class="flex-1 flex flex-col min-w-0">
            ...
            <main class="p-6 max-w-7xl w-full mx-auto space-y-5 flex-1 relative">
                ...
                <div class="flex items-center gap-2 border-b border-slate-200/80 pb-1">
                    ...
                    <a href="users.php?tab=archived" class="px-4 py-2 text-xs font-bold rounded-xl transition-all <?= ($tab ?? '') === 'archived' ? 'bg-rose-900 text-white shadow-2xs' : 'text-rose-700 bg-rose-50 hover:bg-rose-100' ?>">
                        📦 Archived Accounts (<?= count($archivedUsers ?? []); ?>)
                    </a>
                </div>
                            ...
                            <table class="w-full text-left border-collapse text-xs">
                                ...
                                <tbody class="divide-y divide-slate-100 text-slate-700">
                                    <?php if (empty($students)): ?>
                                        <tr>
                                            <td colspan="5" class="py-8 text-center text-slate-400 italic">No active students registered yet.</td>
                                        </tr>
                                    <?php else: ?>
                                        <?php foreach ($students as $s): ?>
                                            <tr class="hover:bg-slate-50/80 transition-colors">
                                                ...
                                                <td class="py-3.5 px-5 text-right space-x-1 whitespace-nowrap">
                                                    <button onclick='openEditStudentModal(<?= json_encode($s); ?>)' class="px-2.5 py-1 text-[11px] font-semibold text-slate-700 bg-slate-100 hover:bg-slate-200 rounded-lg transition-all cursor-pointer">
                                                        Edit
                                                    </button>
                                                    <form method="POST" action="users.php" class="inline" onsubmit="return confirm('Archive student <?= htmlspecialchars(addslashes($s['name'])); ?>?');">
                                                        ...
                                                        <button type="submit" class="px-2.5 py-1 text-[11px] font-semibold text-amber-700 bg-amber-50 hover:bg-amber-100 rounded-lg transition-all cursor-pointer">
                                                            Archive
                                                        </button>
                                                    </form>
                                                </td>
                            ...
                            <table class="w-full text-left border-collapse text-xs">
                                <thead>
                                    <tr class="bg-slate-50/60 text-slate-400 text-[11px] uppercase tracking-wider border-b border-slate-100 font-semibold">
                                        ...
                                        <th class="py-3 px-5">Active Interns</th>
                                        <th class="py-3 px-5 text-right">Actions</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-slate-100 text-slate-700">
                                    <?php if (empty($supervisors)): ?>
                                        <tr>
                                            <td colspan="5" class="py-8 text-center text-slate-400 italic">No active supervisors registered yet.</td>
                                        </tr>
                                    <?php else: ?>
                                        <?php foreach ($supervisors as $sup): ?>
                                            <tr class="hover:bg-slate-50/80 transition-colors">
                                                ...
                                                <td class="py-3.5 px-5 font-bold text-[#0F2854]"><?= intval($sup['assigned_interns']); ?> Intern(s)</td>
                                                <td class="py-3.5 px-5 text-right space-x-1 whitespace-nowrap">
                                                    <button onclick='openEditSupervisorModal(<?= json_encode($sup); ?>)' class="px-2.5 py-1 text-[11px] font-semibold text-slate-700 bg-slate-100 hover:bg-slate-200 rounded-lg transition-all cursor-pointer">
                                                        Edit
                                                    </button>
                            ...
                            <table class="w-full text-left border-collapse text-xs">
                                ...
                                <tbody class="divide-y divide-slate-100 text-slate-700">
                                    <?php if (empty($companies)): ?>
                                        <tr>
                                            <td colspan="3" class="py-8 text-center text-slate-400 italic">No partner companies registered yet.</td>
                                        </tr>
                                    <?php else: ?>
                    ...
                    <div class="bg-white rounded-2xl border border-slate-200/80 shadow-xs overflow-hidden">
                        <div class="overflow-x-auto">
                            <table class="w-full text-left border-collapse text-xs">
                                <thead>
                                    <tr class="bg-slate-50/60 text-slate-400 text-[11px] uppercase tracking-wider border-b border-slate-100 font-semibold">
                                        <th class="py-3 px-5">Account Name</th>
                                        <th class="py-3 px-5">Email</th>
                                        <th class="py-3 px-5">Role</th>
                                        <th class="py-3 px-5">Archived Date</th>
                                        <th class="py-3 px-5 text-right">Actions</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-slate-100 text-slate-700">
                                    <?php if (empty($archivedUsers)): ?>
                                        <tr>
                                            <td colspan="5" class="py-8 text-center text-slate-400 italic">No archived accounts in records.</td>
                                        </tr>
                                    <?php else: ?>
                                        <?php foreach ($archivedUsers as $au): ?>
                                            <tr class="hover:bg-slate-50/80 transition-colors">
                                                <td class="py-3.5 px-5 font-bold text-slate-900"><?= htmlspecialchars($au['name']); ?></td>
                                                <td class="py-3.5 px-5 text-slate-500"><?= htmlspecialchars($au['email']); ?></td>
                                                <td class="py-3.5 px-5">
                                                ...
                                                <td class="py-3.5 px-5 text-right space-x-1 whitespace-nowrap">
                                                    <form method="POST" action="users.php" class="inline" onsubmit="return confirm('Restore user <?= htmlspecialchars(addslashes($au['name'])); ?> back to active?');">
                                                        <input type="hidden" name="action" value="restore_user">
                                                        <input type="hidden" name="user_id" value="<?= $au['id']; ?>">
                                                        <button type="submit" class="px-2.5 py-1 text-[11px] font-semibold text-emerald-700 bg-emerald-50 hover:bg-emerald-100 rounded-lg transition-all cursor-pointer">
                                                            Restore
                                                        </button>
                                                    </form>

                                                    <form method="POST" action="users.php" class="inline" onsubmit="return confirm('⚠️ WARNING: This will permanently delete <?= htmlspecialchars(addslashes($au['name'])); ?> and all related records from the database. This action CANNOT be undone. Proceed?');">
                                                        <input type="hidden" name="action" value="delete_user_permanently">
                                                        <input type="hidden" name="user_id" value="<?= $au['id']; ?>">
                                                        <button type="submit" class="px-2.5 py-1 text-[11px] font-semibold text-rose-700 bg-rose-50 hover:bg-rose-100 rounded-lg transition-all cursor-pointer">
                                                            Delete Permanently
                                                        </button>
                                                    </form>
                                                </td>
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-2xl max-w-md w-full overflow-hidden p-6 space-y-4">
            ...
            <form action="users.php" method="POST" class="space-y-3 text-xs">
                ...
                <div class="flex justify-end gap-2 pt-3">
                    ...
                    <button type="submit" class="px-5 py-2 bg-[#0F2854] text-white rounded-xl font-semibold cursor-pointer">Save Changes</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div id="editSupervisorModal" class="hidden fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center z-50 p-4">
        <div class="bg-white rounded-2xl border border-slate-200 shadow-2xl max-w-md w-full overflow-hidden p-6 space-y-4">
            <h3 class="text-base font-bold text-slate-900">Edit Supervisor Record</h3>
            <form action="users.php" method="POST" class="space-y-3 text-xs">
                <input type="hidden" name="action" value="edit_user">
